...
from imodels.rule_list.bayesian_rule_list.brl_util import default_permsdic, \
    run_bdl_multichain_serial, merge_chains, get_point_estimate, get_rule_rhs, preds_d_t
...
class BayesianRuleListClassifier(BaseEstimator, ClassifierMixin, RuleList):
    """
    ...
    def __init__(self, listlengthprior=3, listwidthprior=1, maxcardinality=2, minsupport=10,
                 alpha=np.array([1., 1.]), n_chains=3, max_iter=50000, class1label="class 1",
    ...
    def _setlabels(self, X, feature_labels=None):
        if feature_labels is None:
            if isinstance(X, pd.DataFrame) and (
                    'object' in str(X.columns.dtype) or 'str' in str(X.columns.dtype)):
    ...
    def _discretize_mixed_data(self, X, y, undiscretized_features=None):
        if not isinstance(X, list):
            X = np.array(X).tolist()
        if undiscretized_features is None:
        ...
        for fi in range(len(X[0])):
            ...
            if isinstance(X[0][fi], numbers.Number) and (
                    len(self.feature_labels) == 0 or len(undiscretized_features) == 0 or
        ...
        if len(self.discretized_features) > 0:
            if self.verbose:
                print(
                    "Warning: non-categorical data found. Trying to discretize. "
    ...
    def _setdata(self, X, y, feature_labels=None, undiscretized_features=None):
        self._setlabels(X, feature_labels)
        X = self._discretize_mixed_data(X, y, undiscretized_features)
        return X, y

    def fit(self, X, y, feature_labels=None, undiscretized_features=None, **kwargs):
    ...
    def discretize(self, X, y):
        ...
        D = pd.DataFrame(np.hstack((X, np.array(y).reshape((len(y), 1)))),
                         columns=list(self.feature_labels) + ["y"])
        self.discretizer = MDLP_Discretizer(dataset=D, class_label="y",
                                            features=self.discretized_features)
        ...
        for i in range(len(self.feature_labels)):
            ...
            if label in self.discretized_features:
                ...
                for j in range(len(self.discretizer._data[label])):
                    toadd = self.discretizer._data.loc[j, label]
                    if pd.isnull(toadd):
                        toadd = 'NULL'
                    column += [label + " : " + toadd]
    ...
    def __str__(self, decimals=1):
        if self.d_star:
            ...
            for i, j in enumerate(self.d_star):
                ...
                s = s.replace('-inf_to_', 'less than ')
                s = s.replace('_to_inf', ' or higher')
